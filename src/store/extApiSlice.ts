...
import {fetchUsers} from "./actions/extApiActions";
...
export const extApiSlice = createSlice({
    ...
    initialState,
    reducers: {
        // usersFetchingSuccess(state, action: PayloadAction<ICoffee[]>) {      // будет вызываться когда подгрузка пользователей произошла успешно
    ...
    extraReducers: {
        [fetchUsers.fulfilled.type]: (state, action: PayloadAction<ICoffee[]>) => {      // Сценарий успешной загрузки
            state.isLoading = false;
            state.error = '';
            state.coffee = action.payload;
        },
        [fetchUsers.pending.type]: (state) => {                                         // Сценарий ожидания
            state.isLoading = true;
        },
        [fetchUsers.rejected.type]: (state, action: PayloadAction<string>) => {         // Сценарий когда произошла ошибка
            state.isLoading = false;
...
//export const {addTrackToMix} = extApiSlice.actions;
